...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        num_test = int(len(df_raw) * 0.15)  # was: 0.2
...
class Dataset_OHLCV(Dataset):
    ...
    def __fix_columns(self, df_raw):
        cols = list(df_raw.columns)
        # =============================================================
        # Bitcoin Minute data: Remove exess columns and rename Volume
        # =============================================================
    ...
    def __read_data__(self):
        ...
        df_raw.columns: ['date', ...(other features), target feature]
        '''
        df_raw = self.__fix_columns(df_raw)

        # Interpolate missing values  # ToDo Do more here? interpolate to a certain limit?
        df_raw = df_raw.interpolate().ffill().bfill()
        # df_raw = self.__handle_nans()

        if self.tech_analysis:
        ...
        # Make sure the date columns is first and the target column is the last
        df_raw = df_raw[['date'] + cols + [self.target]]
        # print(cols)
        num_train = int(len(df_raw) * 0.7)
        num_test = int(len(df_raw) * 0.15)  # was: 0.2
